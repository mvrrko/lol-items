...
import { generatePricingOptions } from '../../utils/quizUtils';
...
export const PricingQuiz: React.FC<PricingQuizProps> = ({
  item,
  version,
  onAnswer,
  timeLimit,
  ...
  const [options] = useState(() => generatePricingOptions(item.gold.total));
  ...
  const handleSubmit = () => {
    ...
    const answer = selectedAnswer !== null ? selectedAnswer.toString() : '';
    setAnswered(true);
    setTimeout(() => {
  ...
  const isCorrect = selectedAnswer === item.gold.total;
    ...
    <div className="max-w-2xl mx-auto">
      <Card>
        ...
        <div className="flex flex-col items-center mb-8">
          ...
          <p className="text-gray-400">What is the total cost of this item?</p>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4 mb-6">
          {options.map((option) => (
            <button
              ...
              disabled={answered}
              className={`p-6 rounded-lg border-2 transition-all text-xl font-bold ${
                answered
                  ? option === item.gold.total
        ...
        {answered && (
          ...
          >
            <p className="text-xl font-bold">
              {isCorrect ? '✓ Correct!' : `✗ Wrong! Correct answer: ${item.gold.total}g`}
            </p>
          </div>
        )}
      </Card>
